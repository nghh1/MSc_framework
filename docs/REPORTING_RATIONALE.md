...
time-series curve uses a solid line without point markers; bar charts use solid fills and scatter
...
2. `cumulative_returns_<family>.png` shows economic performance on the final holdout (or latest outer
   fold) after configured transaction costs and slippage. Dates use concise abbreviated labels.
3. Family-separated cumulative-return figures prevent a dominant benchmark from visually
   compressing differences among learned strategies.
4. `sharpe_ranking.png` shows the principal risk-adjusted ranking and uncertainty.
5. `sharpe_over_time_<family>.png` shows whether rankings persist across successive out-of-sample folds.
...
9. `cost_sensitivity_<family>.png` replays saved positions at common cost levels.
10. `crash_period_cumulative_returns_<family>.png` is generated when daily returns exist. Its year is selected
...
- `trade_timing_summary.csv` and `.md` use actual executed stock-level orders to report trailing
  price location, future returns, favourable/adverse excursion, trading frequency, and directional
  hit rates. Statistics are first computed per run and then averaged across runs. A deterministic,
  frequency-matched random-date control distinguishes genuine timing skill from the market's base
  rate; `directional_hit_lift_5` is the observed five-day hit rate minus that control.
