...
def summarise(equity: pd.Series, returns: pd.Series, positions: pd.DataFrame, turnover: pd.Series) -> dict[str, float]:
    ...
    sortino = 0.0 if not downside_deviation else returns.mean() / downside_deviation * np.sqrt(TRADING_DAYS)
