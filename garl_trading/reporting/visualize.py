...
def _color(name: str) -> str:
    return COLORS.get(name, "#52616B")


def _summary(metrics: pd.DataFrame, confidence: float) -> pd.DataFrame:
    numeric = [
    ...
    if "fold_kind" in metrics and (metrics["fold_kind"] == "final_holdout").any():
        ...
        analysis = (
            metrics.groupby(["baseline", "fold"], as_index=False)[numeric]
            .mean()
        )
...
def _overview(metrics: pd.DataFrame, equity: pd.DataFrame, summary: pd.DataFrame, path: Path):
    baselines = summary["baseline"].tolist()
    colors = [_color(name) for name in baselines]
    ...
    subset = equity[
        (equity["fold_kind"] == chosen_kind)
        & (equity["fold"] == chosen_fold)
        & (equity["repetition"] == 0)
    ]
    for baseline, group in subset.groupby("baseline"):
        ...
        axes[1, 0].plot(
            ...
            group["equity"] / group["equity"].iloc[0],
            label=baseline,
            color=_color(baseline),
            linewidth=1.6,
        )
...
def _fold_stability(metrics: pd.DataFrame, path: Path):
...
def _markdown(summary: pd.DataFrame, metrics: pd.DataFrame, confidence: float) -> str:
    display = summary[
        [
            "baseline", "observations", "sharpe_mean", "sharpe_ci", "cagr_mean",
            "max_drawdown_mean", "turnover_daily_mean",
        ]
    ].copy()
    display["sharpe"] = display.apply(
        lambda row: f"{row.sharpe_mean:.3f} ± {row.sharpe_ci:.3f}", axis=1
    )
    ...
    display = display[
        ["baseline", "observations", "sharpe", "cagr", "max_drawdown", "turnover"]
    ]
    ...
    return (
        ...
        "assumptions. Confidence intervals summarize variation across folds and stochastic seeds.\n\n"
        + table
        + "\n\n"
        "## Reading the figures\n\n"
...
def build_report(run_dir: str | Path, *, confidence: float = 0.95) -> None:
    ...
    summary = _summary(metrics, confidence)
    summary.to_csv(report_dir / "summary.csv", index=False)
    _overview(metrics, equity, summary, report_dir / "overview.png")
    _fold_stability(metrics, report_dir / "fold_stability.png")
    _cost_sensitivity(run_dir, positions, metrics, report_dir / "cost_sensitivity.png")
    (report_dir / "summary.md").write_text(
        _markdown(summary, metrics, confidence),
        encoding="utf-8",
    )
