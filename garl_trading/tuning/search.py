...
def parameters(name: str, trial: optuna.Trial) -> dict:
...
def tune_forecaster(name: str, features: pd.DataFrame, targets: pd.Series, closes: pd.Series,
                    inner_folds: list[Fold], trials: int, seed: int, initial_capital: float,
                    transaction_cost_bps: float, slippage_bps: float, short_borrow_bps_annual: float,
                    rebalance_threshold: float = 0.0, decision_interval: int = 1,
                    target_horizon: int = 1, risk_aversion: float = 10.0,
                    objective_metric: str = "sharpe") -> dict:
    def objective(trial: optuna.Trial) -> float:
        params = parameters(name, trial)
        scores = []
