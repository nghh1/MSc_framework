...
def gae(rewards: list[float], values: list[float], dones: list[bool], 
        next_value: float, gamma: float, gae_lambda: float) -> tuple[np.ndarray, np.ndarray]:
...
def asset_ppo_epoch(model: ActorCritic, state: TradingState, optimizer: torch.optim.Optimizer, 
                    ...
                    gae_lambda: float, update_epochs: int = 4) -> None:
    ...
    for _ in range(update_epochs):
        ...
        loss = -torch.minimum(unclipped, clipped).mean() + 0.5 * nn.functional.mse_loss(predicted_values, return_tensor) - 0.01 * entropy
        optimizer.zero_grad()
        loss.backward()
...
def train_independent_ppo(features: dict[str, pd.DataFrame], closes: dict[str, pd.Series], 
                          ...
                          learning_rate: float, gamma: float, cost_rate: float, seed: int, device: str = "auto", 
                          clip_epsilon: float = 0.2, gae_lambda: float = 0.95) -> RLPolicySet:
    ...
    for _ in range(epochs):
        for ticker in tickers:
            asset_ppo_epoch(models[ticker], states[ticker], optimizers[ticker], rollout_length=rollout_length, 
                            gamma=gamma, rng=randoms[ticker], clip_epsilon=clip_epsilon, gae_lambda=gae_lambda)
    return RLPolicySet("independent", models, scalers, tickers, levels, lookback)


def train_joint_ppo(features: dict[str, pd.DataFrame], closes: dict[str, pd.Series], levels: tuple[float, ...], 
                    lookback: int, epochs: int, rollout_length: int, learning_rate: float, gamma: float, 
                    cost_rate: float, seed: int, device: str = "auto", clip_epsilon: float = 0.2, gae_lambda: float = 0.95) -> RLPolicySet:
    tickers = tuple(features)
    scalers = fit_feature_scalers(features)
    states = make_states(features, closes, scalers, levels=levels, lookback=lookback, cost_rate=cost_rate)
    ...
    for _ in range(epochs):
        ...
        for _ in range(rollout_length):
            ...
            with torch.no_grad():
                ...
                probabilities = torch.softmax(logits, dim=-1).squeeze(0).cpu().numpy()
            selected = [int(rng.choice(len(levels), p=probabilities[i])) for i in range(len(tickers))]
            ...
            values.append(float(value.item()))
            old_logs.append(
                float(sum(np.log(probabilities[i, action] + 1e-8) for i, action in enumerate(selected)))
            )
        ...
        for _ in range(4):
            ...
            objective = torch.minimum(ratio * advantage_tensor, 
                                      torch.clamp(ratio, 1 - clip_epsilon, 1 + clip_epsilon) * advantage_tensor)
            probabilities = torch.softmax(logits, dim=-1)
            entropy = -(probabilities * log_probabilities).sum(axis=-1).mean()
            loss = -objective.mean() + 0.5 * nn.functional.mse_loss(predicted_values, return_tensor) - 0.01 * entropy
            optimizer.zero_grad()
            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
