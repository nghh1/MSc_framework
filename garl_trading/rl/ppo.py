    ...
    for _ in range(epochs):
        ...
        for _ in range(rollout_length):
            ...
            with torch.no_grad():
                ...
                probabilities = torch.softmax(logits, dim=-1).squeeze(0).cpu().numpy()
            selected = [
                int(rng.choice(len(levels), p=probabilities[i]))
        ...
        with torch.no_grad():
            ...
            _, next_value = model(
                torch.tensor(bootstrap_observation, device=device).unsqueeze(0)
            )
        advantages, returns = _gae(
            rewards, values, dones, float(next_value.item()), gamma, gae_lambda
        )
        ...
        for _ in range(4):
            ...
            new_log = log_probabilities.gather(
