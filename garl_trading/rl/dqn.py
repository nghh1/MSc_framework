...
def independent_update(model: QNetwork, target: QNetwork, optimizer: torch.optim.Optimizer,
                       batch: list[Transition], gamma: float) -> float:
...
def train_independent_dqn(features: dict[str, pd.DataFrame], closes: dict[str, pd.Series],
                          ...
                          encoder_dropout: float = 0.0, epsilon_decay_fraction: float = 0.5,
                          target_update_interval: int = 10, turnover_penalty_multiplier: float = 1.0,
    ...
    for i, ticker in enumerate(tickers):
        ...
        models[ticker] = QNetwork(
            observation_size, len(levels), **network_parameters).to(device)
...
def train_joint_dqn(features: dict[str, pd.DataFrame], closes: dict[str, pd.Series],
                    levels: tuple[float, ...], lookback: int, epochs: int, rollout_length: int,
                    learning_rate: float, gamma: float, cost_rate: float, seed: int,
                    ...
                    epsilon_decay_fraction: float = 0.5, target_update_interval: int = 10,
    ...
    model = BranchingQNetwork(
        per_asset_size, len(tickers), len(levels), **network_parameters).to(device)
